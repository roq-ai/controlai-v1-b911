...
import { useRoqClient, useAttackFindFirst } from 'lib/roq';
...
function AttackEditPage() {
  ...
  const { data, error, isLoading, mutate } = useAttackFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: AttackInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.attack.update({
        data: values as RoqTypes.attack,
        where: {
          id,
        },
      });
