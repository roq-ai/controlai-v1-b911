...
import { useRoqPlatformUserProfiles, useModelCount, useDefenseCount, useAttackCount } from 'lib/roq';

import { ModelListPage as ShowCaseList } from 'pages/models';

function DashboardPage() {
  const { session } = useSession();

  const modelStat = useModelCount();

  const defenseStat = useDefenseCount();

  const attackStat = useAttackCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Models',
      result: modelStat?.data,
      isLoading: modelStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Defenses',
      result: defenseStat?.data,
      isLoading: defenseStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Attacks',
      result: attackStat?.data,
      isLoading: attackStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`models`} passHref legacyBehavior>
                  <Button
