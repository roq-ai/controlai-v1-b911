...
import { useRoqClient, useModelFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<ModelInterface, unknown>;
...
export function ModelListPage(props: ModelListPageProps) {
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['organization', 'attack.count', 'defense.count'],
          searchTerm: params.searchTerm,
          searchTermKeys: ['name.contains', 'file_path.contains'],
          ...params.filters,
        },
        'model',
      ),
  ...
  const { data, error, isLoading, mutate } = useModelFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.model.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('attack', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          header: 'Attack',
          accessorKey: 'attack',
          cell: ({ row: { original: record } }: any) => record?._count?.attack,
        }
      : null,
    hasAccess('defense', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'defense',
          header: 'Defense',
          accessorKey: 'defense',
          cell: ({ row: { original: record } }: any) => record?._count?.defense,
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/models/view/${record.id}`} passHref legacyBehavior>
                <Button
                  onClick={(e) => e.stopPropagation()}
                  mr={2}
                  padding="0rem 8px"
