...
import { useRoqClient, useModelFindFirst } from 'lib/roq';
...
function ModelEditPage() {
  ...
  const { data, error, isLoading, mutate } = useModelFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: ModelInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.model.update({
        data: values as RoqTypes.model,
        where: {
          id,
        },
      });
